...
def plot_report():
    ...
    plt.savefig('result-with-length.png')
    plt.show()
...
def plot_best():
    ...
    name_list = ['f1', 'precision', 'recall', 'OOV Recall', 'IV Recall']
    all_result = np.array([f1, precision, recall, oovr, ivr])
    ...
    for k in range(len(models)):
        plt.bar(x, all_result[:, k], width=width, label=models[k], tick_label = name_list, fc=colors[k])
        for i in range(len(all_result[:, k])):
            plt.text(x[i], all_result[:, k][i],'{:.1f}%'.format(all_result[:, k][i] * 100),ha='center',va='bottom',fontsize=10)
